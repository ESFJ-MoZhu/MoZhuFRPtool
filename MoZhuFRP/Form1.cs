        ...
        private async Task ExecuteCommand(string command, bool cmdView)
        {
            ...
            try
            {
                ...
                using (frpProcess = new Process())
                {
                    ...
                    MessageBox.Show($"内网穿透启动成功(大概率成功了,如果不好用请打开控制台输出检查错误，是否有端口占用等问题)！使用地址公网服务器地址:{port}访问（IP可替换为zhuzimiko.com）,已经帮你复制到了粘贴板了~");
                    Clipboard.SetText("公网服务器地址" + port);
                    await tcs.Task;
                }
            }
            catch (Exception ex)
